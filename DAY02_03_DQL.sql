...
SELECT *                -- 모드 칼럼을 의미하는 *, 실제 개발할때는 사용 금지
  FROM DEPARTMENT_T;
...
SELECT LOCATION
 ...
 /*
    주요 조건식 작성 방법
    1. 칼럼 = 값
    2. 칼럼 BETWEEN 값1 AND 값2 -- 사이
    3. 칼럼 IN(값1, 값2) -- 둘 다
    4. 칼럼 IS NULL -- 이 칼럼이 NULL 값을 가지고 있느냐
    5. 칼럼 IS NOT NULL -- 이 칼림이 NOT NULL 값을 가지고 있느냐
    6. 칼럼 LIKE 값 -- 이 값을 포함 된 데이터 
*/
...
SELECT *
  FROM DEPARTMENT_T
 WHERE LOCATION = '서울'; -- LOCATION 칼럼은 UNIQUE하지 않기 떄문에 조회 결과는 2개 이상이다.
