...
CREATE SEQUENCE DEPT_SEQ 
    ...
    NOMINVALUE -- 하한선 없음
    NOCYCLE -- 번호 순환 없음
    CACHE 20 -- 20개의 번호를 미리 생성 (번호를 미리 생성하지 않는 NOCACHE옵션을 사용함)
    NOORDER -- 번호를 순서 없이 이용
;
...
/*
    ...
    UPDATE 테이블명 SET 업테이트할내용, 업테이트할내용, .... WHERE 조건식
*/

-- 1. DEPARTMENT_T에서 부서번호(DEPT_NO)가 3인 부서의 지역(LOCATION)을 '인천'으로 바꾸시오.
UPDATE DEPARTMENT_T
   SET LOCATION = '인천' -- SET절이 등호(=)는 대입 연산자
...
-- 3. DEPARTMENT_T에서 부서번호(DEPT_NO)가 2인 부서를 삭제하시오. (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 참조 무결성 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 삭제가 가능하다.)
DELETE 
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 2; 
